...
# ===== Yeast evaluator path =====
TASK_EVALUATOR_PATH = os.getenv(
    "FUNSEARCH_TASK_EVALUATOR_PATH",
    "/data1/lz/clrs/openevolve/yeast_usl/openevolve_evaluator.py",
)
...
class Sandbox(evaluator.Sandbox):
    ...
    def run(
        ...
        **kwargs
        ...
        real_timeout_seconds = int(os.getenv("FUNSEARCH_EVAL_TIMEOUT", str(timeout_seconds)))
...
INPUT_DIM = int(os.getenv("FUNSEARCH_YEAST_INPUT_DIM", "8"))
NUM_CLASSES = int(os.getenv("FUNSEARCH_YEAST_NUM_CLASSES", "10"))
HIDDEN_DIM = int(os.getenv("FUNSEARCH_YEAST_HIDDEN_DIM", "32"))
DROPOUT = float(os.getenv("FUNSEARCH_YEAST_DROPOUT", "0.1"))
...
def build_mlp_layers(input_dim, num_classes, hidden_dim, dropout):
    """
    Build the classifier layers for the Yeast MLP.
    ...
    5. Input is a dense tabular feature vector for Yeast classification.
    """
...
if __name__ == "__main__":
    model = SimpleMLP(input_dim={INPUT_DIM}, num_classes={NUM_CLASSES}, hidden_dim={HIDDEN_DIM}, dropout={DROPOUT})
...
if __name__ == '__main__':
    ...
    dummy_inputs = {"yeast": None}

    global_max_sample_num = int(os.getenv("FUNSEARCH_MAX_SAMPLES", "20"))
    log_dir = os.getenv("FUNSEARCH_LOG_DIR", "logs/funsearch_yeast_qwen_api")
    ...
    seed_result = seed_sandbox.run(
        ...
        inputs=dummy_inputs,
        test_input="yeast",
        timeout_seconds=int(os.getenv("FUNSEARCH_EVAL_TIMEOUT", "1200")),
    )
