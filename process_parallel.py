...
def _run_iteration_worker(
    ...
    try:
        ...
        # print("parent_id:",parent_id,parent)
        ...
        # print("parent_island:",parent_island)
        # ISLAND_POLICY = {
        #     0:"force decision(s) [B & D] Major Refactor on get_triplet_msgs (LARGE CHANGE)&Major Refactor on PGN (LARGE CHANGE) ",
        #     1:"force decision(s) [B] Major Refactor on get_triplet_msgs (LARGE CHANGE)",
        #     2:"force decision(s) [D] Refactor Topology — LARGE, block-level structural redesign",
        #     3:"force decision(s) [C] Minor Modification on PGN (SMALL CHANGE)",
        #     4:"force decision(s) [A] Minor Modification on get_triplet_msgs (SMALL CHANGE) "
        # }
        # policy_line = ISLAND_POLICY.get(parent_island, "no forced decision")

        policy_header = ''
        # llmh begin
        responses_llmh = ''
        # prompt_llmh = _worker_prompt_sampler.build_prompt(
        #     current_program=parent.code, template_key="LLM_H_REPLY",
        ...
        # )
        # print("prompt_llmh:", prompt_llmh)
        # responses_llmh = asyncio.run(
        #     _worker_llm_ensemble.generate_with_context(
        #         system_message=prompt_llmh["system"],
        #         messages=[{"role": "user", "content": prompt_llmh["user"]}],
        #     )
        # )
        print("responses_llmh:", responses_llmh)
        # llm2 begin
        best_program_overall = island_programs[0]
        current_top_program_code = best_program_overall.code
        current_top_program_metrics = best_program_overall.metrics

        responses_llm2 = ''
        prompt_llm2 = _worker_prompt_sampler.build_prompt(
            current_program=parent.code, template_key="llm2_reply",evolution_summary = responses_llmh,current_top_program_code = current_top_program_code,
            current_top_program_metrics = current_top_program_metrics,

        )
        print("prompt_llm2:",prompt_llm2)
        responses_llm2 = asyncio.run(
            _worker_llm_ensemble.generate_with_context(
                system_message=prompt_llm2["system"],
                messages=[{"role": "user", "content": prompt_llm2["user"]}],
            )
        )
        print("responses_llm2:",responses_llm2)
        # Build prompt
        prompt = _worker_prompt_sampler.build_prompt(
            ...
            template_key="hybrid_user",
            llm2_expert = responses_llm2,
            evolution_summary = responses_llmh,
            current_top_program_code = current_top_program_code,
            current_top_program_metrics = current_top_program_metrics,
        )
        print("prompt:",prompt)
        ...
        #     new_code = parse_full_rewrite(llm_response, _worker_config.language)
        #     if not new_code:
        #         return SerializableResult(error="No valid diffs or code found in response", iteration=iteration)
        #     child_code = new_code
        #     changes_summary = "Full rewrite (auto)"
        from openevolve.utils.code_utils import parse_full_rewrite
        new_code = parse_full_rewrite(llm_response, _worker_config.language)
        if not new_code:
            return SerializableResult(error="No valid code found in response", iteration=iteration)
        child_code = new_code
        changes_summary = "Hybrid (code-only)"
        # ---------- END REPLACE ----------
