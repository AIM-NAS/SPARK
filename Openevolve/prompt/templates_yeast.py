...
with a compatible constructor:
    ...
    SimpleMLP(input_dim: int = 8, num_classes: int = 10)
...
## Task context
This task evaluates a neural classifier on the UCI Yeast dataset.
Each sample is a yeast protein represented by 8 numerical features.
The goal is to predict the protein's subcellular localization class.
...
- optimize:
    combine_score = accuracy-driven score with penalties on loss, MACs, parameter count, and runtime
...
This is a small tabular multiclass classification task.
Therefore:
...
BASE_EVALUATOR_SYSTEM_TEMPLATE = """You are an expert code reviewer for tabular multiclass classifier evolution.
...
- Prefer safe edits, numerical stability, classification quality, and efficiency first.
- Return ONLY valid JSON. No Markdown, no prose outside JSON.
"""

BASE_LLM2_SYSTEM_TEMPLATE = """
You are an expert reviewer for Yeast tabular neural classifiers.
...
- This is supervised multiclass classification on tabular numeric features.
- The network consumes batched dense feature vectors.
...
- Identify harmful patterns for small tabular classification
  (e.g., under-expressive MLP, oversized hidden layers, unstable normalization,
   poor activation choice, weak bottleneck design, wasteful parameter allocation).
...
#
You are LLM1, responsible for evolving the Yeast classifier program.
...
- This is a tabular multiclass classification task on the UCI Yeast dataset.
- Each sample is an 8-dimensional protein feature vector.
- The model predicts subcellular localization classes.
- The evaluator prefers better generalization accuracy, but also penalizes excessive loss, parameter count, MACs, and runtime.
...
- Prefer changes likely to improve evaluator combine_score.
...
- Output must be raw logits shaped (B, num_classes).
...
- avoid oversized models because combine_score penalizes parameters, MACs, and runtime
...
EVALUATION_TEMPLATE = """You are reviewing a tabular multiclass classifier implementation.

Title: Architecture-first Engineering Review for Yeast Classifier Evolution
task_name: yeast_classifier
objective: Improve evaluator combine_score without changing the public interface
...
1.4 evaluator target:
    improve `combine_score`, driven mainly by better test accuracy with penalties on loss, MACs, params, and time
...
4.2 At least ONE entry MUST be category="architecture"
    ...
    and explain WHY it helps tabular multiclass classification.
...
7.2 You may include "combine_score", "final_test_acc", "ood_acc", "final_test_loss", "num_params", "macs", and "time" if controller-supplied.
