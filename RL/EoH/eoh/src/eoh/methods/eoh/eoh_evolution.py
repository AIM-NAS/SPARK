...
+self.prompt_inout_inf+"\n"+"Do not give additional explanations."
    ...
    def _get_alg(self, prompt_content):
        ...
        def parse_response(resp: str):
            ...
            # 1) 提取 algorithm 描述：优先取 {...}，没有就给一个默认描述
            algorithm = re.findall(r"\{(.*)\}", resp, re.DOTALL)
            algorithm = algorithm[0].strip() if len(algorithm) > 0 else "evolved neural architecture"
            ...
            # 4) 如果没有 class，就退化为提取从 def __init__/def forward 开始的方法块
            ...
            # 5) 再退化：如果响应里本身从 class/def 开始，直接全收
