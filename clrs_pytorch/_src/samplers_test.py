...
#
# Strict PyTorch-aligned port of `samplers_test.py`.
#
# Changes vs original test:
# - Remove JAX/Chex dependencies; provide local tree utilities.
# - Import from clrs_pytorch._src.* instead of clrs._src.*.
# - Keep logic, parameterization, and assertions 1:1 with originals.
# ==============================================================================
...
def _tree_map(fn, *trees):
  """Apply fn to leaves of multiple trees with matching structure."""
  if not trees:
    raise ValueError("_tree_map needs at least one tree")
  ref = trees[0]
  # NamedTuple -> dict to keep field names
  if _is_namedtuple(ref):
    keys = ref._fields
    return type(ref)(*[_tree_map(fn, *[getattr(t, k) for t in trees]) for k in keys])
  if isinstance(ref, dict):
    assert all(isinstance(t, dict) and t.keys() == ref.keys() for t in trees), "Mismatched dict keys"
    return {k: _tree_map(fn, *[t[k] for t in trees]) for k in ref}
  if isinstance(ref, (list, tuple)):
    assert all(isinstance(t, (list, tuple)) and len(t) == len(ref) for t in trees), "Mismatched list/tuple lengths"
    out = [_tree_map(fn, *xs) for xs in zip(*trees)]
    return type(ref)(out) if isinstance(ref, tuple) else out
  # Leaf
  return fn(*trees)


def _assert_trees_all_equal(x, y):
  def _cmp(a, b):
    if isinstance(a, np.ndarray) and isinstance(b, np.ndarray):
      np.testing.assert_array_equal(a, b)
      return None
    # Allow numpy scalars / python scalars
    if np.isscalar(a) and np.isscalar(b):
      assert a == b
      return None
    # Fallback for DataPoint etc.
    assert a == b
    return None
  _tree_map(_cmp, x, y)
...
class SamplersTest(parameterized.TestCase):
  ...
  def test_sampler_batch_determinism(self, name):
    ...
    # Validate that datasets are the same.
    _tree_map(lambda a, b: np.testing.assert_array_equal(a, b), feedback_1, feedback_2)
  ...
  def test_sampler_rounding(self, algo_name, track_max_steps):
    ...
    _tree_map(
        lambda a, b: np.testing.assert_array_almost_equal(a, b, decimal=truncate_decimals - 2),
        sample_1,
        sample_2,
    )
    ...
    def check_for_close_equality():
      _tree_map(
          lambda a, b: np.testing.assert_array_almost_equal(a, b, decimal=truncate_decimals + 1),
          sample_1,
          sample_2,
      )
      _tree_map(
...
class AuxiliaryFunctionsTest(parameterized.TestCase):
  ...
  def test_trunc_array(self, input_data, expected_output, truncate_decimals):
    ...
    _assert_trees_all_equal(actual, expected_output)
...
class ProcessRandomPosTest(parameterized.TestCase):
  ...
  def test_random_pos(self, algorithm_name):
    ...
    _assert_trees_all_equal(batch_with_rand_pos, batch_without_rand_pos)
