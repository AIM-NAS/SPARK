...
def graph(A):
  """Constructs a `graph` probe and preserves the input array family."""
  if isinstance(A, np.ndarray):
    I = np.eye(
        A.shape[0],
        dtype=A.dtype if np.issubdtype(A.dtype, np.number) else np.float32
    )
    return ((A + I) != 0).astype(np.float32)

  A_t = _ensure_tensor(A)
  device, dtype = A_t.device, A_t.dtype
  I = torch.eye(A_t.shape[0], device=device, dtype=dtype)
  return ((A_t + I) != 0).to(torch.float32)
...
def predecessor_pointers_to_permutation_matrix(
    ...
    """
    pointers = _ensure_tensor(pointers)
