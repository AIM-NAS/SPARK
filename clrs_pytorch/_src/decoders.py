...
def log_sinkhorn(x: _Array, steps: int, temperature: float, zero_diagonal: bool,
  ...
  Returns:
    Elementwise logarithm of a doubly-stochastic matrix.
  """
  assert x.ndim >= 2
  assert x.shape[-1] == x.shape[-2]

  if noise_rng_key is not None:
    # 某些老版本 PyTorch 的 rand_like 不支持 generator 参数，
    # 因此改用 torch.rand 显式指定 shape/device/dtype。
    U = torch.rand(
        x.shape,
        dtype=x.dtype,
        device=x.device,
        generator=noise_rng_key
    )
