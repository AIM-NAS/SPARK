...
#
# PyTorch/Numpy reimplementation of CLRS samplers.py with identical API & logic.
# - Removes JAX usage; replaces tree_map with NumPy-based stacking/loops.
# - Keeps class/function names, signatures, control flow, and return types.
#
...
try:
    from .. import algorithms  # relative import when used as a subpackage
except Exception:  # pragma: no cover
    from clrs_pytorch import algorithms  # fallback when installed as top-level
...
# CLRS-30 baseline spec. (unchanged)
...
class Sampler(abc.ABC):
  ...
  def __init__(
    ...
    if num_samples < 0:
      ...
      # print('Creating a dataset with %i samples.', num_samples)
  ...
  def next(self, batch_size: Optional[int] = None) -> Feedback:
    ...
    if not torch.is_tensor(lengths):
        lengths = torch.as_tensor(lengths, dtype=torch.int64)  # 长度通常是整型
...
class MatcherSampler(Sampler):
  ...
  def _sample_data(
          self,
          length: int,  # total length = haystack + needle
          length_needle: Optional[int] = None,
          chars: int = 4,
  ):
      if length < 2:
          raise ValueError(f"MatcherSampler requires length >= 2, got {length}")

      # 为了保证 needle 能嵌入 haystack，必须满足：
      # length_haystack >= length_needle
      # 即 length - length_needle >= length_needle
      # 即 length_needle <= length // 2
      max_valid_needle = max(1, length // 2)

      if length_needle is None:
          if length < 5:
              length_needle = 1
          else:
              length_needle = min(length // 5, max_valid_needle)
      elif length_needle < 0:  # randomize needle length
          max_random_needle = min(-length_needle, max_valid_needle)
          length_needle = self._rng.randint(1, high=max_random_needle + 1)
      else:
          length_needle = min(length_needle, max_valid_needle)
      ...
      # 可嵌入位置是 0 ... (length_haystack - length_needle)，总数要 +1
      max_embed_pos = length_haystack - length_needle
      embed_pos = self._rng.randint(0, max_embed_pos + 1)
...
def _batch_io(traj_io: Trajectories) -> Trajectory:
  ...
  # Concatenate along the first axis for each probe index
  num_probes = len(traj_io[0])
  stacked = []
  for i in range(num_probes):
    datas = [traj_io[t][i].data for t in range(len(traj_io))]
    data_cat = np.concatenate(datas, axis=0)
    ref = traj_io[0][i]
    stacked.append(probing.DataPoint(ref.name, ref.location, ref.type_, data_cat))
  return stacked
...
def _batch_hints(
  ...
  # Initialize batched hints with zeros of same shapes
  first_sample = traj_hints[0]
  batched_traj: Trajectory = []
  for i in range(len(first_sample)):
    x = first_sample[i]
    batched_data = np.zeros(time_and_batch + x.data.shape[2:])
    batched_traj.append(probing.DataPoint(x.name, x.location, x.type_, batched_data))
