...
"""Unit tests for `dataset.py` (PyTorch port)."""
...
class DatasetTest(parameterized.TestCase):
  ...
  def test_chunkify(self, name: str, chunk_length: int):
    ...
    n_chunks = int(np.amax(cum_lengths[-1])) // chunk_length + 1
    ...
    all_input = np.concatenate([np.asarray(c.features.inputs[0].data) for c in chunks])
    all_output = np.concatenate([np.asarray(c.outputs[0].data) for c in chunks])
    all_hint = np.concatenate([np.asarray(c.features.hints[0].data) for c in chunks])
