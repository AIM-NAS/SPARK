...
"""Dynamic programming algorithm generators (PyTorch-aligned, logic identical).

Implements:
- Matrix-chain multiplication (`matrix_chain_order`)
- Longest common subsequence (`lcs_length`)
- Optimal binary search tree (`optimal_bst`)

This is a strict port of the original implementation; the only changes are:
  * remove the `chex` dependency (use tiny local rank checks), and
  * switch imports to this package's local `src.probing/specs`.
...
# Import local probing/specs used by the PyTorch port.
# Layout: clrs_pytorch/algorithms/dynamic_programming_torch.py
#         clrs_pytorch/src/{probing.py,specs.py}
from .._src import probing  # type: ignore
...
def _assert_rank1(a: any):
  """Lightweight replacement for chex.assert_rank(..., 1)."""
  if getattr(a, "ndim", None) != 1:
    raise AssertionError("Expected rank-1 array")


def _assert_each_rank1(seq: Sequence[Any]):
  for x in seq:
    _assert_rank1(x)
