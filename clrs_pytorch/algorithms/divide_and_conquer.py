...
"""Divide and conquer algorithm generators (PyTorch-aligned, logic identical).
...
This file is a strict port of the original JAX/TF-agnostic implementation.
We only:
  * remove the `chex` dependency (use a simple rank-assert instead), and
...
# Import the local probing/specs used throughout the PyTorch port.
# Layout: clrs_pytorch/algorithms/divide_and_conquer.py
...
def _assert_rank1(a: _Array):
  # `chex.assert_rank(A, 1)` replacement with identical effect.
  if getattr(a, "ndim", None) != 1:
    raise AssertionError("Expected rank-1 array for A")
