...
from .._src import probing
from .._src import specs
import numpy as np
...
import torch
def assert_rank(x, rank: int):
    """Assert that tensor(s) x has given rank. x can be a tensor or a list/tuple of tensors."""
    ...
    if t.dim() != rank:
        raise AssertionError(f"Expected rank={rank}, got rank={t.dim()} with shape={tuple(t.shape)}")
