...
flags.DEFINE_string('checkpoint_path', '/tmp/CLRS303_20251128',
...
# ==== Global tracker for best test 'pi' ====
BEST_TEST_PI = float('-inf')   # 全局：历史最佳 test 的 pi（越大越好）

def _get_pi_from_stats(stats):
    """
    从 collect_and_eval 返回的 stats 中尽量取出 'pi'（优先）或 'score'（回退）作为标量。
    如果是数组/列表则取均值；拿不到就返回 nan。
    """
    import numpy as np
    ...
    if 'pi' in stats:
        ...
        # 可能是按任务/算法嵌套的 dict，向下一层找
        for _, sub in stats.items():
    ...
    try:
        ...
        try:
            return float(np.array(v, dtype=float).mean())
        except Exception:
            return float('nan')
...
def main(unused_argv):
  ...
  while step < FLAGS.train_steps:
    ...
    if step >= next_eval:
      ...
      global BEST_TEST_PI  # 明确我们要写全局变量
      test_pi_vals = []
      test_stats_all = []
      for algo_idx in range(len(train_samplers)):
          common_extras = {'examples_seen': current_train_items[algo_idx],
          ...
          logging.info('(test) algo %s : %s', FLAGS.algorithms[algo_idx], test_stats_i)
          print('(test) algo %s : %s', FLAGS.algorithms[algo_idx], test_stats_i)
          test_stats_all.append(test_stats_i)
          pi_i = _get_pi_from_stats(test_stats_i)
          if not (np.isnan(pi_i) or np.isinf(pi_i)):
              test_pi_vals.append(float(pi_i))
          curr_test_pi=float(sum(test_pi_vals))
          if curr_test_pi > BEST_TEST_PI:
              BEST_TEST_PI = curr_test_pi
          print('BEST_TEST_PI:', BEST_TEST_PI)
